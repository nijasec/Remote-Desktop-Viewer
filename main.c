...
void startserver(struct arguments *args)
{
    ...
    int recvbuflen = DEFAULT_BUFLEN;
      ...
      printf("\n Application is running on machine port:%d",args->port);
while(TRUE)
...

      {
         ...
         for (i = 0; total>0 && ses!=NULL; i++)
            ...
            //    printf("\nenterinf=g loop");
                    ...
                    {
                        total--;
                          n = recv(ses->client, recvbuf, recvbuflen, 0);
...
int main(int argc, char **argv)
{
  ...
  //socket
     ...
     switch (result->ai_family) {
            ...
            case AF_INET:
              ...
              //  printf("\tIPv4 address %s\n",
                  sprintf(ipstr,"%s",inet_ntoa(sockaddr_ipv4->sin_addr)); 
                break;
     }
     
   //  inet_ntoa(result->ai_family,result->ai_addr, ipstr, sizeof ipstr); 
   ...
   u_long handle= _beginthread(startserver,0,args);
   do{
